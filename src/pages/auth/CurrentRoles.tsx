                    ...
                    <ul className="flex flex-col break-all my-4" key={ role.id }>
                        <li className="inline-flex items-center gap-x-2 py-3 px-4 text-sm font-medium bg-white border text-gray-800 -mt-px first:rounded-t-lg first:mt-0 last:rounded-b-lg ">
                            <div className="md:flex-grow">
                                <h4 className="font-medium text-gray-900 title-font mb-2">{role.position}</h4>
                                <textarea rows={4} cols={40} className=" w-full text-base px-4 py-2 border border-gray-300 focus:outline-none resize-none focus:border-indigo-500" onBlur={ (e) => onEditRole({ id: role.id, position: role.position, description: e.target.value }) } defaultValue={ role.description } />
